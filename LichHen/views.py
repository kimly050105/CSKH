...
def sua_lich_hen(request, id):
    ...
    khach_hang = lich_hen.khach_hang
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for dv in form.cleaned_data.get('dich_vu', []):
                DV_LichHen.objects.create(lich_hen=lich_hen, dich_vu=dv)

            # 📨 Gửi thông báo
            ThongBao.objects.create(
                tieu_de="🔄 Lịch hẹn đã được cập nhật",
                noi_dung=f"Lịch hẹn cho bé {lich_hen.thu_cung.ten_thucung} đã được cập nhật vào {timezone.localtime(lich_hen.thoi_gian).strftime('%H:%M %d/%m/%Y')}.",
                loai='lich_hen',
                nguoi_gui=request.user,
